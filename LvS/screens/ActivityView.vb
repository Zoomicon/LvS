...
#Region "Fields"
 ...
 Protected updatingFocusRow As Boolean =false
...
#Region "Methods"
 ...
 Function Close() As Boolean Implements IActivity.Close
  If Modified Then
			Dim result As DialogResult = MessageBox.Show(My.Resources.STR_ACTIVITY_MODIFIED, My.Resources.STR_QUESTION, MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1)
   Select Case result
...
#Region "Import"
 ...
 Public Sub ImportSubtitles(ByVal thePath As String, ByVal encoding As Encoding) Implements IActivity.ImportSubtitles
		If Not IO.File.Exists(thePath) Then Throw New FileNotFoundException(My.Resources.STR_FILE_NOT_FOUND & ": " & thePath)
  LoadSubtitles(thePath, encoding) 'not copying the subtitles file into the activity, we're storing subtitles into a single format
  Save() 'save the subtitles internally right after importing
 End Sub

 Public Sub ImportDocument(ByVal thePath As String, ByVal encoding As Encoding) Implements IActivity.ImportDocument
		If Not IO.File.Exists(thePath) Then Throw New FileNotFoundException(My.Resources.STR_FILE_NOT_FOUND & ": " & thePath)
  LoadDocument(ImportFile(thePath), IO.Path.GetFileNameWithoutExtension(thePath), encoding)
 End Sub
...
#Region "Export"
 ...
 Public Function ExportSubtitles(ByVal thePath As String, ByVal encoding As Encoding) As Boolean Implements IActivity.ExportSubtitles
  Subtitles.Export(thePath, encoding)
 End Function
...
#Region "Load"
 ...
 Public Sub LoadSubtitles(ByVal thePath As String, ByVal encoding As Encoding) Implements IActivity.LoadSubtitles
  Subtitles.Encoding = encoding
  Subtitles.Source = thePath 'do not talk to the "Subtitles" inner object directly
  CheckSubtitleButtonsEnabled()
 End Sub

 Public Sub LoadDocument(ByVal thePath As String, ByVal theTitle As String, ByVal encoding As Encoding) Implements IActivity.LoadDocument
  Documents.LoadDocument(thePath, theTitle, encoding)
 End Sub
