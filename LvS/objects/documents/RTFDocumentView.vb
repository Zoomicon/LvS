  ...
  Public Overrides Property Source() As String Implements IDocument.Source
   ...
   Set(ByVal value As String)
    If CheckExtension(value, DocumentUtils.EXTENSION_RTF) IsNot Nothing Then
					rtfText.LoadFile(value, RichTextBoxStreamType.RichText)	'this sets Modified=false without calling rtfText_ModifiedChanged
    ...
    Else
     ...
     If IsUnicode(Encoding) Then streamType = RichTextBoxStreamType.UnicodePlainText Else streamType = RichTextBoxStreamType.PlainText
					rtfText.LoadFile(New IO.StreamReader(value, Encoding, True).BaseStream, streamType)	'this sets Modified=false without calling rtfText_ModifiedChanged
					RaiseModifiedChanged()
    End If
    MarkProtectedParts()
    fSource = value
...
#Region "Methods"
  ...
  End Sub
		...
		Public Overrides Sub Export(ByVal thePath As String) Implements models.documents.IDocument.Export
			If CheckExtension(thePath, DocumentUtils.EXTENSION_RTF) IsNot Nothing Then
				rtfText.SaveFile(thePath, RichTextBoxStreamType.RichText)	'this won't affect the Modified flag
			Else
				Dim streamType As RichTextBoxStreamType
				If IsUnicode(Encoding) Then streamType = RichTextBoxStreamType.UnicodePlainText Else streamType = RichTextBoxStreamType.PlainText
				rtfText.SaveFile(thePath, streamType)	'this won't affect the Modified flag
			End If
		End Sub
...
#Region "Events"
  ...
  Private Sub rtfText_LinkClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkClickedEventArgs) Handles rtfText.LinkClicked
   ...
   Catch
				MessageBox.Show(Me, My.Resources.STR_OPEN_URL_FAILED & " " & e.LinkText, My.Resources.STR_ERROR, MessageBoxButtons.OK, MessageBoxIcon.Error)
   End Try
  End Sub
