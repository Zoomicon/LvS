...
#Region "Methods"
  ...
  Public Shared Function DateTimeToSeconds(ByVal theDateTime As DateTime, ByVal baseDateTime As DateTime, ByVal digits As Integer) As Double
   Dim result As Double = theDateTime.Subtract(baseDateTime).TotalSeconds
   If digits <> -1 Then Return Math.Round(result, digits)
   Return result
  End Function

  Public Shared Function SecondsToDateTime(ByVal seconds As Double, ByVal datetimeBase As DateTime, ByVal digits As Integer) As DateTime
   Try
    If digits <> -1 Then seconds = Math.Round(seconds, digits)
    Return datetimeBase.AddSeconds(seconds)
   Catch
    Return datetimeBase 'silently handle exceptions, returning the datetimeBase in that case
   End Try
  End Function

  Public Shared Function DateTimeStrToDateTime(ByVal datetimeStr As String, ByVal baseDateTime As DateTime, ByVal datetimeFormat As String) As DateTime
   ...
   End Try
   result = FixDateTime(result)
   Return result
  End Function

  Public Shared Function DateTimeStrToSeconds(ByVal datetimeStr As String, ByVal baseDateTime As DateTime, ByVal datetimeFormat As String, ByVal digits As Integer) As Double
   Return DateTimeToSeconds(DateTimeStrToDateTime(datetimeStr, baseDateTime, datetimeFormat), baseDateTime, digits)
  End Function

  Public Shared Function SecondsToDateTimeStr(ByVal seconds As Double, ByVal datetimeBase As DateTime, ByVal datetimeFormat As String, ByVal digits As Integer) As String
   Return SecondsToDateTime(seconds, datetimeBase, digits).ToString(datetimeFormat)
  End Function

  Public Shared Function TimeSpanToSeconds(ByVal firstDateTime As DateTime, ByVal secondDateTime As DateTime) As Double
   Return secondDateTime.Subtract(firstDateTime).TotalSeconds
  End Function
