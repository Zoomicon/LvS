...
#Region "Fields"
  ...
  Protected fShowReadOnly As Boolean = False
  Protected fReadOnlyChecked As Boolean = False
   ...
   Select Case (msg)
    ...
    Case WfINITDIALOG
     ...
     Dim labelHandle As Integer = CreateWindowEx(0, "STATIC", "mylabel", (WS_VISIBLE _
                     Or (WS_CHILD Or WS_TABSTOP)), point.X, (point.Y + Y_OFFSET + 6), 200, 100, parent, 0, 0, 0)
     SetWindowText(labelHandle, EncodingLabel)
     Dim fontHandle As Integer = SendMessage(fileTypeWindow, WfGETFONT, 0, 0)
     SendMessage(labelHandle, WfSETFONT, fontHandle, 0)
     ...
     Dim comboHandle As Integer = CreateWindowEx(0, "ComboBox", "mycombobox", (WS_VISIBLE _
                     Or (WS_CHILD _
                     Or (CBS_HASSTRINGS _
                     Or (CBS_DROPDOWNLIST Or WS_TABSTOP)))), point.X, (point.Y + Y_OFFSET), (rightPoint.X - point.X), 100, parent, 0, 0, 0)
     SendMessage(comboHandle, WfSETFONT, fontHandle, 0)
     'and add the encodings we want to offer 'Birb
     Dim i As Integer = 0
     For Each e As EncodingInfo In Encoding.GetEncodings
      SendMessage(comboHandle, CB_ADDSTRING, 0, e.DisplayName)
      If e.GetEncoding.Equals(Encoding) Then SendMessage(comboHandle, CB_SETCURSEL, CType(i, Integer), 0) 'preselect the given encoding
      i += 1
     Next e
    ...
    Case WfNOTIFY
     ...
     If (nmhdr.Code = CDN_FILEOK) Then
      ...
      fEncoding = Encoding.GetEncodings(SendMessage(fComboHandle, CB_GETCURSEL, 0, 0)).GetEncoding 'Birb
     End If
